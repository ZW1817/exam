...
public class SysUserController {
    ...
    @CrossOrigin("*")
    public ResponseUtil insertCourse(@RequestParam("name") String name){
        KemuModel kemumodel = new KemuModel();
        kemumodel.setName(name);
        int row = kemuService.insert(kemumodel);
        if (row > 0) {
            return ResponseUtil.ok("课程添加成功，请重新刷新界面！");
        }
        return ResponseUtil.error("添加失败！");
    }

    @RequestMapping("/insertClass")
    public ResponseUtil insertClass(BanjiModel banjiModel){
        banjiModel.setTno("无");
        int row = banjiService.insert(banjiModel);
        if (row > 0) {
            return ResponseUtil.ok("班级添加成功，请重新刷新界面！");
        }
        return ResponseUtil.error("添加失败！");
    }

    @RequestMapping("/updateClass")
    public ResponseUtil updateClass(BanjiModel banjiModel){
        int row = banjiService.update(banjiModel);
        if (row > 0) {
            return ResponseUtil.ok("教师修改成功，请重新刷新界面！");
        }
