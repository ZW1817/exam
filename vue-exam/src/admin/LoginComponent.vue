    ...
    submitForm() {
      ...
      axios({
        ...
        if(response.data.code==1){
          // window.alert("登录成功！");
          let no = response.data.data;
          if(role == 0){
            that.$router.push("/admin");
            // window.location.href="http://127.0.0.1:5173/adminPage/no";
          }else if(role == 1){
            // window.location.href="http://127.0.0.1:5173/stuShow/no";
          }else{
            // window.location.href="http://127.0.0.1:5173/teacherShow/no";
          }
        }
      }).catch(function(err){
